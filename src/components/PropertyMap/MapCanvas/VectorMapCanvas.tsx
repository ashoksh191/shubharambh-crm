...
export const VectorMapCanvas: React.FC<VectorMapCanvasProps> = ({
  ...
  const handleZoomToPlot = (plot: EnhancedPlot) => {
    if (transformRef.current) {
      ...
      setTransform(-plot.x * 1.6 + 650, -plot.y * 1.6 + 450, 2.4, 400, 'easeOut');
    }
  };
        ...
        {({ zoomIn, zoomOut, resetTransform }) => (
          <>
            ...
            >
              ...
              >
                {/* 4K Official Architectural PDF Map Image */}
                <image
                ...
                {/* Interactive SVG Plot Polygons (No overlapping artificial headers or double text) */}
                <g>
                  {plots.map((plot) => (
                    <PlotPolygon
                      key={plot.id}
